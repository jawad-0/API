...
const postQuery = [ "INSERT INTO UserData (u_id, name, age, gender) VALUES (@u_id, @name, @age, @gender)",
                    "INSERT INTO Album (u_id, photos) VALUES (@u_id, @photos)" ];
...
app.post("/post", async (req, res) => {
  try {
    ...
    for (const { id, name, age, gender, image } of userData) {
      console.log("Data received:", { id, name, age, gender, image });
      await pool.connect();
      for (const query of postQuery) {
      await pool
        .request()
        .input("u_id", sql.Int, id)
